...
int main() {
  ...
  const double t1_max = json["spec"]["t1max"].number_value();
  ...
  if (sdip.n_rows < 2) {
    printf("Error! K3 does not survive in monomer!");
  } else {
    corr3rdK3(t1_max, w2_max, w3_max, nt1, nt2, nt3, dt, staticErr, nk, sdip, pdip, bdip, sch_hei[0], s, b, h);
  }

...
public:
  ...
  void remImag(cx_cube &d_ddos, const cx_cube &ddos, const double t);
  ...
  void equilibrium(cx_cube &ddos, const double dt = 0.005, const double err = 2.0e-8, const int nk = 10, const string &method = string("Imag")) {
    if (method == "Imag") {
      EqSolverImag(ddos, dt, err, nk);
    } else if (method == "SCI3") {
  ...
  void EqSolverImag(cx_cube &ddos, const double dt = 0.005, const double err = 2.0e-8, const int nk = 10);

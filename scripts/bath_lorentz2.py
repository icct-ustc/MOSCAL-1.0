...
def jwsdr (omg,jsdr):
    ...
    return lams*gams*omg*(1.0/((omg-omgs)**2+gams**2)+1.0/((omg+omgs)**2+gams**2))
   ...
   #print 'rn = %16.6e'%rn, 'tn = %16.6e'%tn
    ...
    for m in xrange(nmod):
        ...
        for isdr in xrange(nsdr):
            ...
            jind = iind+1
            delr[m] += 2*rn*lams*gams/temp
            expn[iind] = gams+1.J*omgs
            expn[jind] = gams-1.J*omgs
            z1, z2 = -1.J*expn[iind], -1.J*expn[jind]

            etal[iind] = lams*(omgs-1.J*gams)*fBose(z1/temp,pole,resi,rn,tn,exbe)
            etal[jind] =-lams*(omgs+1.J*gams)*fBose(z2/temp,pole,resi,rn,tn,exbe)
